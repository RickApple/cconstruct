...
int main() {
  ...
  auto platform_x86 = cc.createPlatform(EPlatformTypeX86);
  ...
  auto l = cc.createProject("my_library", CCProjectTypeStaticLibrary);
  {
    const char* files[] = {"src/library.c"};
    cc.project.addFiles(l, "Source Files", countof(files), files);
  }

  auto b = cc.createProject("my_binary", CCProjectTypeConsoleApplication);
  {
